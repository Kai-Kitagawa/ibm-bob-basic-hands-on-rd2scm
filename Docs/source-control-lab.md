# LAB4 - ソース管理

[プログレスバー画像: LAB1 → LAB2 → LAB3 → LAB4（強調）]

![Lab 4概要図](画像パス予定)
<!-- 
視覚的説明図:
- Bobキャラクターのイラスト
- 吹き出し: 「Lab3までで作成・改善したコードを、コミットメッセージとプルリクエストで管理します」
- スクリーンショット: Git差分とコミットメッセージ作成画面
-->

Lab3までで作成したFAQサイトをさらにブラッシュアップし、変更をGitで管理します。

---

## 事前準備

このLabを始める前に、以下を確認してください：

✅ Lab1〜Lab3を完了している  
✅ GitHubアカウントを作成済み  
✅ リポジトリをFork済み  
✅ ローカルに変更差分がある状態

> [!NOTE]
> GitHub操作が難しい場合は、講師の画面で流れを確認するだけでも問題ありません。

---

## セッションの切り替え

「新しいタスクを開始」を押して、セッションを切り替えてください。

![新しいタスクを開始ボタン](画像パス予定)

---

## 1. 変更内容の確認

まず、Lab1〜Lab3で行った変更を確認します。

### 手順：

1. Bob IDEのソース管理パネルを開く
2. 変更されたファイルを確認
3. 差分を確認

![ソース管理パネルと差分表示](画像パス予定)

### 変更ファイル：

- **faq-site/index.html** - FAQサイトの基本構造
- **faq-site/css/variables.css** - スタイル変数とテーマカラー
- **faq-site/css/layout.css** - レイアウトとレスポンシブデザイン
- **faq-site/js/theme.js** - テーマ切り替え機能

### ユーザーのアクション：

- 変更ファイル一覧を確認
- 各ファイルの差分を確認

---

## 2. コミットメッセージの作成

IBM Bobのコミットメッセージ作成支援機能を使います。

### 手順：

1. ソース管理パネルでコミットメッセージ入力欄を確認
2. 「✨」アイコン（AI生成）をクリック
3. Bobが差分を分析してメッセージを提案

![コミットメッセージ生成ボタン](画像パス予定)

### Bobが提案するメッセージの例：

```
feat: FAQサイトの初期実装とテーマ切り替え機能を追加

- HTML/CSS/JavaScriptでFAQサイトを実装
- ダークモード切り替え機能を追加
- レスポンシブデザインに対応
- セキュリティ改善を反映
```

![生成されたコミットメッセージ](画像パス予定)

### ユーザーのアクション：

- 提案されたメッセージを確認
- 必要に応じて編集
- 「コミット」をクリック

![コミット完了画面](画像パス予定)

### 確認：

✅ コミットが作成された  
✅ コミットメッセージが適切

---

## 3. 変更をリモートにプッシュ

コミット後、変更をGitHubにプッシュします。

### 手順：

1. ソース管理パネルで「変更の同期」をクリック
2. または「プッシュ」をクリック

![プッシュボタン](画像パス予定)

### 確認：

✅ リモートブランチに変更がプッシュされた  
✅ GitHubリポジトリで変更が確認できる

---

## 4. プルリクエストの作成

IBM Bobのプルリクエスト作成支援機能を使います。  
プルリクエストの作成には2つの方法があります。

### 方法1: Bob IDEのメニューから作成

1. Bob IDEで「プルリクエストを作成」を選択
2. Bobが変更内容を分析してPR説明文を生成

![プルリクエスト作成メニュー](画像パス予定)

### 方法2: スラッシュコマンドで作成

1. チャット画面で `/create-pr` と入力
2. Bobが変更内容を分析してPR説明文を生成

![/create-prコマンド実行画面](画像パス予定)

### PR説明文の生成：

Bobが変更内容を分析して、PR説明文を提案します。

![PR説明文生成画面](画像パス予定)

### 提案される説明文の例：

```markdown
## 概要
FAQサイトの初期実装とテーマ切り替え機能を追加しました。

## 変更内容
- FAQサイトの基本構造を実装（HTML/CSS/JavaScript）
- ダークモード切り替え機能を追加
- レスポンシブデザインに対応
- コードレビューで指摘されたセキュリティ問題を修正

## 確認ポイント
- [ ] FAQサイトが正しく表示される
- [ ] テーマ切り替えが動作する
- [ ] レスポンシブデザインが機能する
- [ ] セキュリティ問題が解決されている

## スクリーンショット
（必要に応じて追加）
```

### ユーザーのアクション：

- 説明文を確認・編集
- 「Create pull request」をクリック

![PR作成完了画面](画像パス予定)

---

## 5. 確認

プルリクエストが正しく作成されたか確認します。

### 確認ポイント：

✅ PRタイトルが適切  
✅ PR説明文が分かりやすい  
✅ 変更ファイルが正しく含まれている  
✅ レビュー依頼ができる状態

![完成したPR画面（GitHub上）](画像パス予定)

---

## Lab4タスクの完了

### Lab4の完了

コミットメッセージとプルリクエストの作成が完了しました！  
IBM Bobを活用することで、変更内容を適切に記録し、チームに共有できます。

![タスク完了画面](画像パス予定)

🎉 **タスク完了**

---

## 次のステップ

これでLab1〜Lab4のすべてが完了しました！  
要件定義から実装、レビュー、ソース管理まで、一連の開発フローを体験できました。

→ [まとめと次のステップ](https://r-nakayamasan.github.io/bob-workshop-draft/06_summary/)

---

## 関連リンク

- [READMEに戻る](../README.md)
- [準備と設定ガイド](preparation-and-setup.md)
- [コミットメッセージ機能](https://bob.ibm.com/docs/ide/features/commit-messages)
- [プルリクエスト機能](https://bob.ibm.com/docs/ide/features/pull-requests)